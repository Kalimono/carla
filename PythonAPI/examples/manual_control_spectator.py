...
"""
CARLA Manual Control with Spectator Following (No Rendering)

This script spawns and controls a vehicle using keyboard input, but does NOT
render anything in Python. Instead, the Unreal Editor spectator camera follows
the vehicle, and all rendering happens in the editor.
...
class World(object):
    ...
    def update_spectator(self):
        ...
        if self.player is not None and self.spectator is not None:
            # Get vehicle transform
            vehicle_transform = self.player.get_transform()
            
            # Apply local offset (driver's eye position) to avoid occlusion by vehicle model
            # x=1.6m forward, z=1.7m up in vehicle's local coordinate system
            offset = carla.Location(x=1.6, z=1.7)
            
            # Transform the offset from local to world coordinates
            offset_world = vehicle_transform.transform(offset)
            
            # Create spectator transform with offset position but same rotation as vehicle
            spectator_transform = carla.Transform(offset_world, vehicle_transform.rotation)
            
            # Set spectator to follow vehicle with offset
            self.spectator.set_transform(spectator_transform)

    def tick(self, clock):
        # Spectator automatically follows hero vehicle in Unreal Engine
        # No need to update from Python
        # print(f"Player rolename: {self.player.attributes.get('role_name', 'N/A')}")
        pass
...
def game_loop(args):
    ...
    pygame.display.set_caption('CARLA Spectator Control (Input Only)')
    ...
    try:
        ...
        print('Spectator in Editor will follow the vehicle')
        print('=' * 60)
...
def main():
    argparser = argparse.ArgumentParser(
        description='CARLA Manual Control with Spectator Following')
